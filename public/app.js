...
// $.get('/troll.svg').success(function(data){
// 	var svg = $(data).find('svg');
// 	//console.log(svg, insertTo);
// 	insertTo.html(svg);

// 	var troll = Snap('#image');
// 	console.log(troll)
...
// var troll = Snap('#image');
// Snap.load('/troll.svg', function(data){
var commentarios = new Firebase('https://troll-comments.firebaseio.com/commentarios');
...
formulario.submit(function(e) {
    e.preventDefault();
    var data = {
        texto: $('textarea', formulario).val()
    };


    if (data.texto.length > 4) {
        var comentario = commentarios.push(data);
        $('textarea', formulario).val('')
        return false;
    }
    alert('Necesito un comentario más largo.')

    return false;
})

//var comentario = commentarios.push({texto:'llalalal', nota: 'otro'});
//comentario.set({texto:'llalalal', nota: 'otro'});
Snap.load('/troll.svg', function(svg) {
    var texto = $(svg.select('#textooo').node);
    var lista = $('ul#lista');
    commentarios.on("value", function(snapshot) {
    	lista.html('');
        var comentariosarray = _.map(snapshot.val(), function(one, key) {
        	var uncomentario = $('<li title="'+key+'">'+one.texto+'</li>');
        	lista.append(uncomentario);
        	uncomentario.click(function(){
        		if ($('textarea', formulario).val().trim() == 'borrar')
        			commentarios.child(key).remove();
        	});
            return one.texto;
        });
        ...
        //comentariosarray.push('ÚLTIMO COMENTARIO >');
        //comentariosarray = comentariosarray.reverse();
        texto.text(comentariosarray.reverse().join(' - '));
...
// usersRef now refers to the 'users' Firebase location
...
// 	//console.log(data);
// 	var path = data.select('#Layer_6').select('path');


// 	troll.text(50,50,'holas ajajajajaj');
